...
//  |  | parcourt $childOf{id}
        foreach ($arrayOfArrayOfChild[arrayChild($comment->id)] as $commentLevel1) :
            $compteur++;
//  |  |  | crée un décalage pour l'affichage des réponses
            echo '<div class="col-xs-offset-1">';
//  |  |  | affiche le commentaire (panel)
            showComment($commentLevel1, $compteur, $postId);
//  |  |  | teste si $childOf{id} existe
            if (isset($arrayOfArrayOfChild[arrayChild($commentLevel1->id)])) {
//  |  |  |  | parcourt $childOf{id}
                foreach ($arrayOfArrayOfChild[arrayChild($commentLevel1->id)] as $commentLevel2) :
                    $compteur++;
//  |  |  |  |  | crée un décalage pour l'affichage des réponses
                    echo '<div class="col-xs-offset-1">';
//  |  |  |  |  | affiche le commentaire (panel)
                    showComment($commentLevel2, $compteur, $postId);
//  |  |  |  |  | teste si $childOf{id} existe
                    if (isset($arrayOfArrayOfChild[arrayChild($commentLevel2->id)])) {
//  |  |  |  |  |  | parcourt $childOf{id}
                    foreach ($arrayOfArrayOfChild[arrayChild($commentLevel2->id)] as $commentLevel3) :
                        $compteur++;
//  |  |  |  |  |  |  | crée un décalage pour l'affichage des réponses
                        echo '<div class="col-xs-offset-1">';
//  |  |  |  |  |  |  | affiche le commentaire (panel)
                        showComment($commentLevel3, $compteur, $postId);
//  |  |  |  |  |  | fin de parcours
                        echo '</div>';
                    endforeach;
//  |  |  |  |  | fin test
